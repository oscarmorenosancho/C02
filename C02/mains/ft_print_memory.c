...
/*   Updated: 2022/08/19 12:03:17 by omoreno-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_mem_to_hexcode(void *mem, char *buf, unsigned int bytes, int a_c)
{
	unsigned int	i;
...
void	ft_format_line(char *addr, char *buf, unsigned int size)
{
	...
	{
		if ((i * 2) < size)
			ft_mem_to_hexcode((void *)byte_addr, buf_offset, 2, 1);
		else
		buf_offset += 5;
		byte_addr += 2;
		i++;
